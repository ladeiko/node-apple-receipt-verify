...
function getSubscriptionExpireDate(data) {
  ...
  if (data[REC_KEYS.EXPIRES_DATE]) {
    return Math.ceil((new Date(data[REC_KEYS.EXPIRES_DATE])).getTime()/1000);
  }
  return 0;
}

function getSubscriptionPurchaseDate(data) {
  if (data[REC_KEYS.PURCHASE_DATE_MS]) {
    return parseInt(data[REC_KEYS.PURCHASE_DATE_MS], 10);
  }
  if (data[REC_KEYS.PURCHASE_DATE]) {
    return Math.ceil((new Date(data[REC_KEYS.PURCHASE_DATE])).getTime()/1000);
  }
  return 0;
}
...
module.exports = {
  ...
  validate: function (options, done) {
    ...
    tasks.push(function (taskCallback) {
      validatePurchase(options.receipt, { secret: options.secret }, function (err, response) {
        if (err) {
          taskCallback(err);
        }
        else {
