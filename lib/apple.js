...
function validatePurchase(receipt, options, cb) {
  ...
  const isRetryable = (data) => {
    const flag = _.at(data, 'is-retryable')[0];
    if (_.isString(flag)) {
      return flag.toLowerCase() === 'true' || flag.toLowerCase() !== '0';
    }
  ...
  const req = https.request(options, (res) => {
    ...
    res.on('end', () => {
      try {
        ...
        console.error('Apple Receipt Validation statusCode=', res.statusCode, 'response=', response);
  ...
  validate: function (options, cb) {
    ...
    if (options.device) {
      ...
      if (checkreceipt_exists) {
        ...
        logWarn('node-apple-receipt-verify: [NOTE] receipt will not be verified with vendor identifier!')
      }
    }
