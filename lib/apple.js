...
function isValidConfigKey(key) {
  return ['secret', 'ignoreExpired', 'environment', 'verbose', 'extended', 'ignoreExpiredError', 'excludeOldTransactions', 'requestDefaults'].indexOf(key) >= 0;
}
...
function readConfig(configIn) {
  ...
  Object.keys(configIn).forEach(function (key) {
    if (isValidConfigKey(key)) {
      ...
      console.warn('WARN: Unsupported node-apple-receipt-verify option "' + key + '" found');
    }
  });
}
...
function validatePurchase(receipt, options, cb) {
  ...
  var tasks = _.reduce(options.environment, function (t, e) {
    if (e === 'production'){
      t.push(tryProd);
    }
    else {
...
function getPurchaseData(purchase, options) {
  ...
  if (purchase.receipt[REC_KEYS.IN_APP]) {
    ...
    for (var i = 0, len = list.length; i < len; i++) {
      ...
      var tid = item[REC_KEYS.TRANSACTION_ID];
      ...
      if (options.extended) {
        ...
        if (typeof item.is_in_intro_offer_period !== 'undefined') {
...
module.exports = {
  ...
  config: function (options) {
    ...
    const result = Joi.validate(options, schema, { allowUnknown: true, convert: true });
    if (result.error) {
      throw result.error;
    }
  ...
  validate: function (options, cb) {
    ...
    if (result.error) {
      ...
      if (_.isString(result.error.message)) {
        var match = /\[(.+)\]/.exec(result.error.message);
        if (match) {
        ...
        if (err) {
          ...
          try {
            if (isValidated(response)) {
              var purchaseDataList = getPurchaseData(response, options);
